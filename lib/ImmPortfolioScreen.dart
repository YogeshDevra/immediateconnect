...
import 'package:immediateconnectapp/ImmStaticApiConfig.dart';
import 'package:percent_indicator/linear_percent_indicator.dart';
import 'package:syncfusion_flutter_charts/charts.dart';
import 'ImmChartScreen.dart';
import 'ImmConDatabase.dart';
...
class ImmPortfolioScreen extends StatefulWidget {
  const ImmPortfolioScreen({super.key});


  @override
  State<ImmPortfolioScreen> createState() => _ImmPortfolioScreenState();
}

class _ImmPortfolioScreenState extends State<ImmPortfolioScreen> {
  final immDatabase = ImmConDatabase.instance;
  String coinName = '';
  num size = 0;
  ...
  Future<void> callImmCrypto() async {
    ...
    var uri = '${ImmStaticApiConfig.ImmApiUrl}/Bitcoin/resources/getBitcoinCryptoListLoser?size=0&currency=USD';
    print(uri);
    if (await ImmStaticApiConfig.imm_internetConnection()) {
      try {
        ...
        if(response.statusCode == 200) {
          ...
          if(mounted) {
            if (data['error'] == false) {
              ...
              ImmStaticApiConfig.imm_toastMessage(message:'Under Maintenance');
              setState(() {
                loadingPage = false;
              });
            }
      ...
      ImmStaticApiConfig.imm_toastMessage(message: 'No Internet');
      setState(() {
        loadingPage = false;
      });
    }
    ImmStaticApiConfig.imm_internetConnection();
  }
  ...
  Future<void> callingGraphApi(String name) async {
    if(portfolio.isNotEmpty) {
      ...
      var uri = '${ImmStaticApiConfig.ImmApiUrl}/Bitcoin/resources/getBitcoinCryptoGraph?type=$imType&name=$coinName&currency=USD';
      if (await ImmStaticApiConfig.imm_internetConnection()) {
        try {
  ...
  _updateAllSaveCoinsToLocalStorage(ImmPortfolioCrypto bitcoin) async {
    if (ImmUpdateCoinEditingController!.text.isNotEmpty && ImmUpdateCoinEditingController!.text != 0) {
      ...
      Map<String, dynamic> row = {
        ImmConDatabase.columnName: bitcoin.name,
        ImmConDatabase.columnRateDuringAdding: bitcoin.rateDuringAdding,
        ImmConDatabase.columnCoinsQuantity:
        double.parse(ImmUpdateCoinEditingController!.value.text),
        ImmConDatabase.columnTotalValue: (adf) * (bitcoin.rateDuringAdding),
      };
                      ...
                      Container(
                        ...
                        child: Column(
                          ...
                          children: [
                            InkWell(
                              onTap:(){
                                Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (BuildContext context) => ImmChartScreen(portfolioCoin!.numberOfCoins, portfolioCoin!.name,rate,actualPercentage,cap,volume,differenceRate)));
                              },
                              child: Container(
                                height: 45,
                                width: 126,
                                ...
                                child:Row(
                                  children: [
                                    ...
                                    SizedBox(width: 15,),
                                    Text(ImmAppLocalizations.of(context)!.translate("bitai-21")!,style:GoogleFonts.inter(textStyle:TextStyle(fontSize: 16,fontWeight: FontWeight.w500,color: Color(0xff1A202C) )
                                    ),)
