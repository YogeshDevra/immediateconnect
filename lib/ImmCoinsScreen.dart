...
class ImmCoinsScreen extends StatefulWidget{
  const ImmCoinsScreen ({super.key});

  @override
  State<ImmCoinsScreen> createState() => _ImmCoinsScreenState();
}
class _ImmCoinsScreenState extends State<ImmCoinsScreen>{
  ...
  _addAllSaveCoinsToLocalStorage(ImmCrypto cbitIndex) async {
    ...
    if (portfolio.isNotEmpty) {
      ...
      if (bitcoinLocal != null) {
        Map<String, dynamic> row = {
          ImmConDatabase.columnName: cbitIndex.symbol,
          ImmConDatabase.columnPerRate: cbitIndex.perRate,
          ImmConDatabase.columnRateDuringAdding: cbitIndex.rate,
          ImmConDatabase.columnCoinsQuantity:
          double.parse(coinAddEditingController!.value.text) +
              bitcoinLocal.numberOfCoins,
          ImmConDatabase.columnTotalValue:
          double.parse(coinAddEditingController!.value.text) *
              (cbitIndex.rate!) +
    ...
    print('cap');
    print(cbitIndex.cap);
    Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (BuildContext context) => ImmChartScreen(double.parse(coinAddEditingController!.value.text), cbitIndex.symbol!, cbitIndex.rate, cbitIndex.perRate!,cbitIndex.cap!,cbitIndex.volume!,cbitIndex.differRate!)));
  }
